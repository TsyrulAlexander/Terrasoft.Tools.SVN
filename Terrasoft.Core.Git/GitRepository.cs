...
namespace Terrasoft.Core.Git {
	public class GitRepository {
		public static async Task<GitReleaseInfo> GetLatestReleaseInfoAsync(string repoOwner, string repoName, string token = null) {
			using (var client = GetGithubHttpClient(token)) {
				var response = await client.GetAsync($"repos/{repoOwner}/{repoName}/releases/latest");
